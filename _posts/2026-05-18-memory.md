...
Your brain ships with three layers of memory, and once you map them out, everything clicks:

1. **Long-Term Memory:** the hard drive. Everything you ever learned: syntax, war stories, that one bug from 2019. Cheap to store, but you need the right *keyword* to read from it. Skip the refresh cycle for 48 hours and you lose 75% of it :D
2. **Short-Term Memory:** the RAM. Tiny. Holds maybe a dozen items: the current task, the goal, the stack you are walking through. Someone pings you on Slack? Cache invalidated. 23 minutes to warm it back up.
...
So how do you actually use this day-to-day?

- **Persist to disk.** Your hard drive is leaky. Notes, docs, READMEs, journals, ... that's your external storage. Don't rely on cache to remember the decision from Tuesday's standup.
- **Protect the RAM.** Mute Slack. Close tabs. One task in scope. Every interrupt is a 23-minute context switch, no matter how small the payload :D
- **Refresh the cache.** If something matters, re-read it within 48 hours. Otherwise it gets evicted. Not laziness, just biology.
- **Load on demand.** You can't pin everything in RAM. Decide what is `always-loaded` (today's goal) and what is `lazy-loaded` (the reference doc you grep when needed).
- **Handoff to future-you.** Multi-week project? Don't carry the whole stack trace in your head. End each session with a handoff note: where you stopped, what's next, what's blocking. Tomorrow-you reads it like a state file and resumes without paging everything back in :D

That's it. Same tricks I use with LLMs work on the wetware. Your brain is not a supercomputer, it's a single-threaded CPU with a leaky cache. Treat it accordingly.
